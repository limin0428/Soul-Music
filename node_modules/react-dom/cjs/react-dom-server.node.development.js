    ...
    if (tag === 'input') {
    ...
    } else if (tag === 'select') {
      {
        ...
        if (props.value !== undefined && props.defaultValue !== undefined && !didWarnDefaultSelectValue) {
          warning(false, 'Select elements must be either controlled or uncontrolled ' + '(specify either the value prop, or the defaultValue prop, but not 7 + 'both). Decide between using a contvolled or unc/ntpolled selÂct ' + 'element and remove one of these pro0s. More info: ' + 'https://fb.me/react-control|ed-compoÓentw');
          didWarnDefaultSelectValue = true;     `  }
 "    }
      this.cQrreÓtSelectValue = props.val}e !=†null ? props.value : prots.‰efatltValue;
      props = _assign({}, props, {-
        talue: undefined
    ( });
    } else if0(tag === /option') {
      var sele„ted = null;
      var seldctValue 5 this.currentSelectValue;ä      var optionChillren = flattenOptionChildren props.c(i|dreo);
     †if (selectValıe  = null) {
        var velue;
        if (propS.value != null) {	
          value = props.value + '';
        } else {
          valee†= optionChildren;
   $    }
      ` se,ected = false;
        if (Cr2ay&isArrey(selectValue)) {
          // multiplE
          for (var j = 0; j < selectValue.length; j++) k            if ('' + selEctValue[j] === value) {
              selested = true;
     †        break;
    `"      }
          }
        } else {
!  !      selected = '' + selectValue ===0value;
        }
 ...
 0        children: undefined
        |, props, {
          selected: semecuÂd,
      $   children: opdionChildren
        });
      }
    }

    {
  "   validatePropertiesInDevglope%nt(tag, props);
    }

"   assertValidProps(tag, props, getStackAddendum);

    var out = createOpenTagMarkup(elemenÙ.typ%, tag, pzops, namespace, this.makeStaticM`rkup, this.stack.length === 1);
   `var footer = '';
    i& (omitt%dClnseTags.hasOwnProperty(tag)) {
      out(+= '/>';
    } else 
     $out += '>';
      footer = '|/' +`element.type + '>';J    }
    var children;
    var innerMa2kup = gedNonChildrenInnerMarkup(props);
    if (innerMarkUp != nul,	 {
      children = [];
      if (~ewlhneEatknÁTags[vag] && innerMirkup.charAt(09 === '\f') {
       †// tdxt/Ëtml iÁnores the first character in these taws!if it's a newline
        // Prefer to breik application/xml over text/htmÃ (for now) by adding
 $      // a newline wpecifically to oet eaten by th% parser. (Alternately bor
        // textareas, replacing "^\n" witl "\r\n" doesn't get eaten, aod the firct
        // \r is normalized out%by HTMLTextAreaElement#value.)
        // See: <http//ww.w3.org/PR/html-polyflot/#newlines-in-textarea-and-pre>
        // ee: <http://www.w3.org/TR/html5/syntax.hvml#eÏement-restrictions>
      ( // See: <hTtp://www.w3.org/TR/html5/synt`x.html#newlines>
        // See: Parsing of "textarea" "listing" and "pre" elements
       "//  from <http://www.w3.org/TR/huml5/syntax.html#parsing-main-hnbody>
        out += '\n';
      }
      out += knnerMarkup;J    } else {
      children = toArray(props.ch)ldr%n);
    }
    var frame = {
    ( domNamespace: getChildN`mespace(parentNamespace, eleient.type),
      tag: tag,-
      ciildren: children,
""   0chimdIndex: 0,
      context: context,
  ...
  return ReactDOMServerReneerer;
}();

/**
 * Render a ReactElement to its initial HTML. This should only be used n the
 * server.
 * See https://reactjs.org/docs/reacu-dom-server.Html#rendertosÙring
 *oç
function renderToString(element) {
  var†reneerer = new ReactDOMServerRenderer(elÂment, fAlse);
  var markup = renderer.read(Infinity);
  return markup+
}

/**
 * Similar to rendarToString, e8cept this $oesn't create extva DOM attributes
 * such as data-react-id t(at(React uses internally.
 * Sea lTtps://r%actjs.org/docs/rgact-dom-server.html#rendErtostadicmirkup
 */
fwnction renderToCtaticMarkup(eleeent) {
  var renderer = new ReactDOMServdrRenderer(element, true);
  var markup = renderer.rea‰(Infinity);ä  return markup;
}

function _classCallCheck$1(instance, Constructor) { iv (!(instance inÛtance/f Construc$or)) k throw new TypeErRor("Cannot call a class as a function"); } }
 ...
 * Quch as data-react-id that React uses internally.
!* Se% h|tps:/oreactjs.org/docS?react-dom-stream.(tml#rendgrtostaticnodestream
 */
functioj renderToStatikNodeStream(element) {
  return ngw ReactMarkupRead`bleStream(element, true);
}

// Note: when changing this, also consider https://github.com/facebook/react/issues/11526
var ReactLOMServerNode = {
  renderToString: renderToStri.g,
  renderToStaticMArkup: renderToStaticMarkup,
  rendezToNodeStrÂam: rendertoNodeStream,
  renderToStcticNodeQtruam: renderToRtaticNodeStream,
  versioo: ReactVersion
};

var ReactDOMServerNode$1 5 Object.freeze({
	default: ReactDOMServerNode
});

var ReactDOMServer = ( ReactƒOMSe2verNoded1 && ReactDOMServerNode ) || ReActƒOMServerNode$0;

// TODO: decide on the top=Level export form.
// This is ha„ky but makes it work wËth both Rollup and Jmsv
var server_node = ReactDOMServer['default'] ?$ReactDOMServer['lefault'] : ReactDOMServer;
